...
cl_int InicializarEntornoOCL(EntornoOCL_t *entorno) {
    ...
    //kernel
    entorno->kernel = clCreateKernel(entorno->programa, "med_ocl_mpi", &error);
    if (error!=CL_SUCCESS){CodigoError(error);return error;}
...
void ControlDeWI(int &filas, int &num_workitems, int ne){

    if( (filas%num_workitems) != 0){
        ...
        while( (filas%num_workitems) != 0){
            if(num_workitems <= filas) {
                num_workitems += 1;
            }else{
                num_workitems -= 1;
            }
...
*/
cl_int ocl(int columnas,int *m,int *v, int debug, int ne, EntornoOCL_t entorno, int num_workitems, int workitems_por_workgroups, int size, int myrank) {
    int filas = (columnas+1)/size;

    //Control de errores, arriba explicado en la funcion
    ControlDeWI( filas, num_workitems, ne); //Respecto a ocl asecas, ahora paso el numero de filas que le corresponden a este proceso computar

    int i,
        totalSize=columnas*(columnas+1), // tamano total de la matriz.
        contador=0,
        tamOut = filas; //tamano del vector salida, filas que le corresponden a este proceso
    ...
    size_t despl=num_workitems*myrank;
    ...
    for(int p=0;p<totalSize;p++){   //copio la matriz original  a una matriz copia para tener con que comparar en el kernel, debido a que varios WI
        matrizCopia[p] = m[p];      //estan interactuando con la matriz original ordenando las filas y hay problemas
    }
    ...
    if (error!=CL_SUCCESS){CodigoError(error);return error;}
    error=clSetKernelArg(entorno.kernel,4,sizeof(int),&filas);
    if (error!=CL_SUCCESS){CodigoError(error);return error;}
...
//    if (error!=CL_SUCCESS){CodigoError(error);return error;}
    ...
    //ejecucion kernel
    error = clEnqueueNDRangeKernel (entorno.cola, entorno.kernel, 1, &despl, &NumWorkItems, NULL, 0, NULL, &EventoExec);
    if (error!=CL_SUCCESS){CodigoError(error);return error;}
...
//    if (error!=CL_SUCCESS){CodigoError(error);return error;}
    ...
    /*
     * Si meto 4 procesos, me mete 2 ceros al final???? pero el vector salida de cada proceso va bien
    printf("\nvector\n");
    escribirvector( tamOut, out);
    printf("\n\n");
     */
...
// **************************************************************************
	...
	MPI_Bcast(&num_problems, 1, MPI_INT, 0, MPI_COMM_WORLD);//Recibo numproblems del proceso 0
...
// **************************************************************************
        ...
        MPI_Bcast(&columnas, 1, MPI_INT, 0, MPI_COMM_WORLD);//Recibo columnas del proceso 0

        if(myrank != 0){
            m = new int[(columnas+1)*columnas];
        }
        MPI_Bcast(m,(columnas+1)*columnas,MPI_INT, 0,MPI_COMM_WORLD);//recibo la matriz del proceso 0

        int *V = new int[(columnas+1)/size];

        error = ocl(columnas,m,V,debug,i,entorno,num_workitems,workitems_por_workgroups,size,myrank);

        MPI_Gather(V,(columnas+1)/size,MPI_INT,v,(columnas+1)/size,MPI_INT,0,MPI_COMM_WORLD);
